...
class Extra
{
	...
	{
		$name=$this->ezpk($fileName['name']);
	    $ext=strrchr($name, '.');
        $tmp_name = $fileName['tmp_name'];
        $dir_picture = $path.$name;
        $valables = array('.jpg','.JPG','.PNG','.png','.pdf','.docx','.doc','.xlsx','.xls');
	...
	public function urlHashEncode($content)
	{
			$encode = (28281*$content)+3458;
			return $encode;
	}

	public function urlHashDecode($content)
	{
			$decode= ($content-3458)/28281;
			return $decode;
	}
